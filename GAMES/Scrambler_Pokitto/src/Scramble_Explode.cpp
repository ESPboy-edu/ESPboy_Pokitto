...
void Game::renderShockwave(uint16_t distTravelled, int16_t viewY) {
    ...
    for (Shockwave &shockwave : this->shockwaves.shockwaves) {
        ...
        if (shockwave.getActive()) {
            ...
            PD::setColor(color);
            PD::drawCircle(shockwave.getX() - distTravelled, shockwave.getY() - viewY, shockwave.getRadius());        
...
void Game::renderParticles(uint16_t distTravelled, int16_t viewY) {
    ...
    for (Part &particle : this->particles.particles) {
        ...
        if (particle.getActive()) {
            ...
            if (static_cast<uint8_t>(particle.getSize()) <= 1) {
                ...
                PD::setColor(color);
                PD::drawPixel(particle.getX() - distTravelled, particle.getY() - viewY);

            }
            else {

                PD::setColor(color);
                PD::fillCircle(particle.getX() - distTravelled, particle.getY() - viewY, static_cast<uint8_t>(particle.getSize()));
