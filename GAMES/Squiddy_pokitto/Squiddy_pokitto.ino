...
/*
extern PROGMEM const int16_t lil_map[];   // table of all the level maps
extern PROGMEM const int16_t tile_table[];    // table of colors for the map screen
extern PROGMEM const uint8_t *bitmap_table[];  // table of pointers to the bitmaps for the game screen
extern PROGMEM const uint8_t squiddy[];        // bitmap for squddy, the main character
extern PROGMEM const uint8_t grid[];              //bimap for the grid )(used on the title screen)
extern PROGMEM const uint8_t ball[];          // bitmap for the ball
extern PROGMEM const uint8_t heart[];         // bitmap for the heart, used in a level description
extern PROGMEM const uint8_t face[];          // bitmap for the face, used in a level descriiption
extern PROGMEM const uint8_t cw[];            // bitmap for the copyright symbol
extern PROGMEM const const char *msg_table[64];   // table of text messages for each level
*/
...
PROGMEM static ldata leveldata[NUM_LEVELS] = {
...
int xfirst;
    ...
    for (; (y < height) && (j < 18); y+= step, j++, index += 4)
    {
        ...
        for (i = rowstart; (i < 16) && (x < width) ; i++, x += step)
        {
            ...
            {
                if (color > 3)
                    PD::drawBitmap(x,y,bitmap_table[color] + aframe);
                else
                    PD::drawBitmap(x,y,bitmap_table[color]);
            }
        }
    }
...
int xfirst;
    ...
    for (; (y < height) && (j < 18); y+= step, j++, index += 4)
    {
        ...
        for (i = rowstart; (i < 16) && (x < width) ; i++, x += step)
        {
            ...
            if (color_index > 0)
            {
                PD::color = tile_table[color_index];
                PD::fillRectangle(x,y,step,step);
...
void draw_textbox(int x, int y,int boxlen, int boxheight, int bcolor, int frameColor)
{
            ...
            PD::color = bcolor;
            PD::fillRectangle(x-2,y-3,boxlen,boxheight);
            PD::color = frameColor;
            PD::drawRectangle(x-3,y -4,boxlen+1,boxheight+2);
...
//  PD::width = width; // full size
//  PD::height = height;

  PC::setFrameRate(20);
  PD::load565Palette(def565palette);
};
...
void loop(){
    if (PC::update()) {
            ...
            if (PC::buttons.repeat(BTN_UP,0)){
    			yworld++;
            }
            if (PC::buttons.repeat(BTN_DOWN,0)){
    			yworld--;
            }
            if (PC::buttons.repeat(BTN_LEFT,0)){
    			xworld++;
            }
             if (PC::buttons.repeat(BTN_RIGHT,0)){
    			xworld--;
            }
#endif
        switch(state)
    ...
    case S_TITLE:
        {
            ...
            for (i = 0 ; i < 120 ; i = i + 10)
            {
                for(j = 0; j < 90 ; j= j + 10)
                {
                    PD::drawBitmap(i,j,grid);
                }
            }
            draw_textbox(10,10,12 * 6, 12, 0, C_DARKGREEN);
            PD::color = C_GREEN;
            PD::setCursor(10,10);
            PD::print("Aphrodite's");
    ...
    case S_MAP:
        {
            ...
            if (msg_table[level *2] != 0)
                PD::print(msg_table[level * 2]);
            PD::setCursor(3,12);
            if (msg_table[(level *2) + 1] != 0)
                PD::print(msg_table[(level * 2) + 1]);
#endif
            if (level == 9)
            {
                PD::drawBitmap(16, 3,heart);
                PD::drawBitmap(16,12,heart);
            }
               if (level == 18)
            {
                PD::drawBitmap(22, 3,face);
            ...
            PD::color = C_RED;
            PD::drawLine(sx - 3,sy -3,sx + 3,sy + 3);
            PD::drawLine(sx - 3,sy +3,sx + 3,sy - 3);

               if (PC::buttons.repeat(BTN_A,0) ||
                   PC::buttons.repeat(BTN_LEFT,0) ||
                   PC::buttons.repeat(BTN_RIGHT,0))
            {
                if (release_count > 1)
                {
                    if (PC::buttons.repeat(BTN_A,0))
                    ...
                    else if (PC::buttons.repeat(BTN_LEFT,0))
                    {
                        if (level > 1) level = level -1;
                    }
                    else if (PC::buttons.repeat(BTN_RIGHT,0))
                    {
    ...
    case S_LOST:
    {
        ...
        ballsx = ballwx + xworld;
            ...
            PD::invisiblecolor = 17 ;
            draw_world(xworld,yworld,10,(level -1) * 72,animframe * 52);

            PD::invisiblecolor = 15 ;

            PD::color = 0;
            PD::drawLine(squidsx + 5,squidsy + 5,ballsx+5,ballsy+5);

            PD::drawBitmap(squidsx,squidsy,squiddy);

            PD::drawBitmap(ballsx,ballsy,ball);
                  PD::color = 0;
            if (state == S_WON) boxlen = 6 * 8; else boxlen = 6 * 9;
            if (squidsy > 50) cury = 20; else cury = 70;

            PD::setCursor(30,cury);
            draw_textbox(30,cury,boxlen,12,C_BLACK,C_DARKGREEN);
            //PD::color = 0;
            //PD::fillRectangle(28,cury-3,boxlen,12);
            //PD::color = C_DARKGREEN;
            //PD::drawRectangle(27,cury -4,boxlen+1,14);
            ...
            else
            {
             ...
             if (PC::buttons.repeat(BTN_A,0)) //if button a is pressed
            {
                if (release_count > 4)
                {
